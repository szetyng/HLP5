...
///output is Result<Parse<Instr>,string> option
...
// type DataPath is in CommonData
let executeAnyInstr (ins: Instr) (d: DataPath): Result<DataPath, ErrRun> =
    let execute =
        match ins with
        | IDP ins'  -> DP.execute ins' d |> runErrorMap
        | IMEM ins' -> Memory.execute ins' d |> runErrorMap
    execute d